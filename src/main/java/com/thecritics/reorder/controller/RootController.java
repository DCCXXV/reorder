...
@PostMapping("/reorder/addElement")
public String reorderaddElement(@RequestParam String elementTextInput, HttpSession session, Model model) {
    List<List<String>> orderState = orderService.getOrderState(session);
    String trimmed = elementTextInput.trim();
 ...
 * @return El nombre del fragmento de la vista "createOrder :: #elementsContainer".
 */
@PostMapping("/reorder/deleteElement")
public String reorderdeleteElement(@RequestParam String elementTextBadge, HttpSession session, Model model) {
    log.debug(elementTextBadge);
    List<List<String>> orderState = orderService.deleteElement(elementTextBadge, session);
    model.addAttribute("orderState", orderState);
 ...
 * @returnEl nombre de la vista "createOrder".
 */
@PostMapping("/reorder/addTier")
public String reorderaddTier(Model model, HttpSession session, HttpServletResponse response) {
    List<List<String>> orderState = orderService.getOrderState(session);
 ...
 * @return El nombre de la vista "createOrder".
 */
@PostMapping("/reorder/keepElementsAndDeleteLastTier")
public String reorderkeepElementsAndDeleteLastTier(HttpSession session, Model model) {
    List<List<String>> orderState = orderService.keepElementsAndDeleteLastTier(session);
    model.addAttribute("orderState", orderState);
    return "reorder";
}
 ...
 * @return El nombre de la vista "createOrder".
 */
@PostMapping("/reorder/deleteLastTier")
public String reorderdeleteLastTier(HttpSession session, Model model) {
    List<List<String>> orderState = orderService.getOrderState(session);
    Integer elementCount = (Integer) session.getAttribute("elementCount");
...
public String reorderupdateOrderState(@RequestParam String orderStateJson, HttpSession session, Model model) {
    try {
        log.debug("Recibido orderStateJson: {}", orderStateJson);
        List<List<String>> newOrderState = objectMapper.readValue(orderStateJson, new TypeReference<List<List<String>>>() {});
        log.debug("Enviado newOrderState: {}", newOrderState);
...
public String reorderPublishOrder(@RequestParam String title, @RequestParam String author, HttpSession session, Model model) {
    if (title == null || title.isEmpty()) {
        return "error";
    }
